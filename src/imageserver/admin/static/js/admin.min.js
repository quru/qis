...
alert("You must enter a password for the user.");return false;}if(!validate_isempty("password")&&!validate_length("password",8)){form_setError("password");
alert("The password must be 8 characters or longer.");return false;}if($2("passwordconf")&&$2("password")&&$2("passwordconf").value!==$2("password").value){form_setError("password");
form_setError("passwordconf");alert("The passwords entered do not match.");return false;}return true;
};UserEdit.onSubmitError=function(a,c){GenericPopup.enableButtons();var b=getAPIError(a,c);if(b.status==APICodes.ALREADY_EXISTS){form_setError("username");
alert("This username is already in use, please choose another.");}else{alert("Sorry, your changes were not saved.\n\n"+b.message);
}};var GroupEdit={};GroupEdit.onInit=function(){GenericPopup.initButtons();setAjaxJsonForm("editform",GroupEdit.validate,GenericPopup.defaultSubmitting,GroupEdit.onSubmitSuccess,GroupEdit.onSubmitError);
