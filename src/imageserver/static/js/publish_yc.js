...
}new Request.JSON({url:c.replace("/0/","/"+b+"/"),onSuccess:function(f,e){a.empty();Publisher.setTemplateInfo(f.data);
...
}var d=function(f,e){var g=a[f];return((g!==undefined)&&(g!==null)&&(g!==0))?g:e;};if(Publisher.showingHelp){Publisher.toggleHelp();
}$$("input").each(function(g){var f=Publisher.webFieldToTemplateKey(g.name);if(g.type==="checkbox"){g.checked=(d(f,false)===true);
}else{if(g.type==="color"){var e=d(f,"");g.value=(e&&e!=="auto"&&e!=="none")?e:"#ffffff";}else{g.value=d(f,"");
}}});$$("select").each(function(g){var f=Publisher.webFieldToTemplateKey(g.name);if(f!=="template"||b){var h=d(f,"");
for(var e=0;e<g.options.length;e++){if(g.options[e].value===h){g.selectedIndex=e;return;}}g.selectedIndex=0;
}});if(b){Publisher.templateSpec={};Publisher.templateSpecKV={};$("template_fields").empty();}var c=d("fill","");
if(c==="auto"){$("publish_field_autofill").checked=true;}if(c==="none"){$("publish_field_transfill").checked=true;
}Publisher.clearWarnings();Publisher.onUnitsChanged(false);Publisher.resetCropping();};Publisher.webFieldToTemplateKey=function(a){switch(a){case"src":return"filename";
...
break;default:var c=0;break;}return Math.round(c*a);};Publisher.setTemplateInfo=function(a){var c=a.template,d=$("publish_field_template"),b=d.options[d.selectedIndex].getProperty("data-id");
if(!b){b=""+PublisherConfig.default_template_id;}if(a&&(a.id===parseInt(b))){Publisher.templateSpec=c;
Publisher.templateSpecKV=Publisher.templateToKV(c);Publisher.resetUI(false,Publisher.templateSpecKV);
