...
Publisher.onTemplateChanged();if($("crop_image").complete){Publisher.initCropping();}};Publisher.initSpecs=function(){var c=$("preview_image").getProperty("src"),b=c.indexOf("?"),a=c.substring(b+1).cleanQueryString().replace(/\+/g," ");
...
}Publisher.onChange();}};Publisher.refreshPreview=function(){var g=function(f){try{return parseInt(f,10);
}catch(t){return 0;}};var r=function(f){return f&&(f!=="false")&&(f!=="0");};if(++Publisher.previewImageRC>1){return;
}var m=["width","height","colorspace","format","attach","xref","stats"];var d=Publisher.imageSpec,l=Object.clone(Publisher.previewSpec);
for(var q in d){if(!m.contains(q)){l[q]=d[q];}}var s=Publisher.templateSpec,k={format:s.format?s.format.value:undefined,width:s.width?s.width.value:undefined,height:s.height?s.height.value:undefined,autosizefit:s.autosizefit?s.autosizefit.value:undefined,autocropfit:s.autocropfit?s.autocropfit.value:undefined};
var e=(d.format||k.format||"").toLowerCase();if(["gif","jpg","jpeg","pjpg","pjpeg","png","svg"].contains(e)){l.format=e;
}var a=g(d.width)||g(k.width)||0,h=g(d.height)||g(k.height)||0,o=(d.autosizefit!==undefined)?r(d.autosizefit):r(k.autosizefit),n=(d.autocropfit!==undefined)?r(d.autocropfit):r(k.autocropfit),p=g(l.width),b=g(l.height),i=false;
if(a&&(a<=p)){l.width=a;i=!h||(h<=b);}if(h){if(h<=b){l.height=h;i=!a||(a<=p);}else{i=false;}}if(a&&h){if(!i){var j=a/h;
if(j>=1){l.height=Math.round(p/j);}else{l.width=Math.round(b*j);}}if(!o){delete l.autosizefit;}if(o){delete l.autocropfit;
}}else{if(n){delete l.autocropfit;}}var c=Publisher.previewURL+"?"+Object.toQueryString(l);if($("preview_image").getProperty("src")===c){return Publisher.refreshedPreview();
}$("preview_image").setStyle("display","");$("preview_error").setStyle("display","none");$("preview_mask").setStyle("display","block");
$("preview_image").setProperty("src",c);};Publisher.refreshedPreview=function(a){$("preview_mask").setStyle("display","none");
