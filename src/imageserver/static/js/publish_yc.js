...
Publisher.init=function(){addEventEx("crop_image","load",Publisher.refreshedCropImage);addEventEx("crop_image","load",Publisher.resetCropping);
...
Publisher.onTemplateChanged();if($("crop_image").complete){Publisher.resetCropping();}};Publisher.initSpecs=function(){var c=$("preview_image").getProperty("src"),b=c.indexOf("?"),a=c.substring(b+1).cleanQueryString().replace(/\+/g," ");
...
}Publisher.refreshWarnings();Publisher.refreshPreview();Publisher.refreshPublishOutput();};Publisher.resetUI=function(b,a){if(a===undefined){a={};
}var f=function(j,i){var k=a[j];return((k!==undefined)&&(k!==null)&&(k!=="")&&(k!==0))?k:i;};if(Publisher.showingHelp){Publisher.toggleHelp();
}$$("label").each(function(i){i.removeClass("highlight");});$$("input").each(function(k){var j=Publisher.webFieldToTemplateKey(k.name);
if(k.type==="checkbox"){k.checked=(f(j,false)===true);}else{if(k.type==="color"){var i=f(j,"");k.value=(i&&i!=="auto"&&i!=="none")?i:"#ffffff";
}else{k.value=f(j,"");}}});$$("select").each(function(l){var k=Publisher.webFieldToTemplateKey(l.name);
if(k!=="template"||b){var m=f(k,"");if(typeof m==="string"){m=m.toLowerCase();}for(var j=0;j<l.options.length;
j++){if(l.options[j].value.toLowerCase()==m){l.selectedIndex=j;return;}}l.selectedIndex=0;}});if(b){Publisher.templateSpec={};
Publisher.templateSpecKV={};$("template_fields").empty();}var c=f("fill","");if(c==="auto"){$("publish_field_autofill").checked=true;
}if(c==="none"){$("publish_field_transfill").checked=true;}for(var e in a){var h=f(e,null);if(h&&(h!=="#ffffff")){var g=$("publish_field_"+e);
if(g){var d=g.getParent().getFirst("label");if(d){d.addClass("highlight");}}}}Publisher.clearWarnings();
Publisher.onUnitsChanged(false);Publisher.resetCropping();};Publisher.webFieldToTemplateKey=function(a){switch(a){case"src":return"filename";
...
Publisher.templateSpecKV=Publisher.templateToKV(d);Publisher.resetUI(false,Publisher.templateSpecKV);
var e=b.id===PublisherConfig.default_template_id;a.empty();a.set("text",e?PublisherText.default_template_labels:PublisherText.template_labels);
a.grab(new Element("br"));a.grab(new Element("button",{text:PublisherText.reset_changes,style:"margin-top: 0.3em",events:{click:function(){Publisher.resetUI(false,Publisher.templateSpecKV);
...
Publisher.popupHelp.showAt(b,a);Publisher.showingHelp=true;}return false;};Publisher.resetCropping=function(){if(Publisher.crop!==undefined){$("crop_fix_aspect").removeEvent("change",Publisher.changeAspectRatio);
