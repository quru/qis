...
}.bind(this)).send();};ImgSlideshow.prototype.onDataReady=function(g){if(g&&(g.status==200)){for(var a=0;
a<g.data.length;a++){this.options.images.push({src:this.options.folder+g.data[a].filename});}}if((this.options.images.length>0)&&(this.imageSize.width>0)&&(this.imageSize.height>0)){for(var a=0;
a<this.options.images.length;a++){var e=this.options.images[a],b={};b=QU.merge(b,this.options.params);
b=QU.merge(b,e);delete b.url;delete b.server;b.width=this.imageSize.width;b.height=this.imageSize.height;
b.autosizefit=1;b.strip=1;if(!b.format){b.format="jpg";}var d=e.server?e.server:this.options.server;var c=d+"image?"+QU.ObjectToQueryString(b);
var f=document.createElement("img");f._onload=this.onImageReady.bind(this);f.addEventListener("load",f._onload,false);
f.setAttribute("data-index",a);f.src=c;f.style.margin="0";f.style.padding="0";f.style.verticalAlign="top";
this.imageEls.push(f);}}else{this.setMessage("");}};ImgSlideshow.prototype.onImageReady=function(c){var b=c.target;
