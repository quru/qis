...
if(c!==undefined){this.options=Object.merge(this.options,c);}this.events=b;this.options.server=this._add_slash(this.options.server);
this.options.folder=this._add_slash(this.options.folder);this.options.images.each(function(d){if(d.server){d.server=this._add_slash(d.server);
}}.bind(this));this.firstIdx=0;this.thumbIdx=-1;this.thumbnails=[];this.touchAttrs={last:{x:0,y:0}};this.ctrEl=document.id(a);
this.ctrEl.empty();this.elements={};this.create_ui();this.layout();}GalleryView.prototype.init=function(){this.setMessage("Loading gallery...");
