...
if(p!=null){var m=typeOf(p);if(m=="function"){p=p.apply(null,Array.convert(j));}else{if(m=="object"){p=Object.clone(p);
...
if(!n){continue;}p=n.get(r,j,q);if(p!=null){return p;}}return"";},inherit:function(g,h){g=Array.convert(g);
...
},units:{ms:Function.convert(1),second:Function.convert(1000),minute:Function.convert(60000),hour:Function.convert(3600000),day:Function.convert(86400000),week:Function.convert(608400000),month:function(t,s){var u=new a;
...
}}}},toElement:function(){return this.picker;},setTitle:function(b,a){if(!a){a=Function.convert;}this.titleText.empty().adopt(Array.convert(b).map(function(d,c){return typeOf(d)=="element"?d:new Element("div.column",{text:a(d,this.options)}).addClass("column_"+(c+1));
...
}var a=Array.convert(b),d=Array.convert(c),e=[].append(a).combine(d),i=this;var g=function(l){var j=i.options.blockKeydown&&l.type=="keydown"&&!(["tab","esc"].contains(l.key)),k=l.type=="keydown"&&(["tab","esc"].contains(l.key)),m=l.target.get("tag")=="a";
...
}var f=Array.convert(c),e=Array.convert(a),d=[].append(f).combine(e),b=this;if(!d.length){d=b.attachedElements;
...
["month","year","decades"].some(function(j,h){return(f.startView==j)&&(f.startView=e[h]);});f.canAlwaysGoUp=f.canAlwaysGoUp?Array.convert(f.canAlwaysGoUp):[];
