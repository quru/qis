...
*/
"use strict";
var Upload={enhancedUpload:false,droppedFiles:[]};Upload.init=function(){addEventEx("uploadform","submit",Upload.onSubmit);
...
$("dropzone").ondragend=Upload.onDragEnd;$("dropzone").ondragleave=Upload.onDragEnd;$("dropzone").ondrop=Upload.onDragDrop;
$("submit").disabled=false;Upload.enhancedUpload=Upload.isEnhancedUpload();if(!Upload.enhancedUpload){$("dropfiles").setStyle("display","none");
...
Upload.droppedFiles=[];};Upload.onPathKey=function(a){if(a.code!=9){$("path_index_manual").checked=true;
...
Upload.onDragDrop=function(c){c.stopPropagation();c.preventDefault();if(c.dataTransfer&&c.dataTransfer.files&&c.dataTransfer.files.length){for(var b=0;
b<c.dataTransfer.files.length;b++){var a=c.dataTransfer.files.item(b);if(!Upload.fileInList(a,Upload.droppedFiles)){Upload.droppedFiles.push(a);
...
if(validate_isempty("files")&&validate_isempty("directory")&&!Upload.droppedFiles.length){form_setError("files");
...
alert("You must enter the name of the folder to upload to");return false;}if(Upload.droppedFiles.length&&!Upload.enhancedUpload){form_setError("files");
...
setTimeout(function(){Upload.setProgress(0);},1000);}};Upload.runEnhancedUpload=function(){if($("files").value===""){$("files").disabled=true;
}if($("directory").value===""){$("directory").disabled=true;}var b=$("uploadform"),c=new FormData(b),d=new XMLHttpRequest();
for(var a=0;a<Upload.droppedFiles.length;a++){c.append("files",Upload.droppedFiles[a]);}b.api_json_as_text.value="false";
d.upload.addEventListener("progress",function(f){Upload.setProgress(f.lengthComputable?Math.round(f.loaded*100/f.total):-1);
...
},false);d.open(b.method,b.action);d.send(c);$("files").disabled=false;$("directory").disabled=false;
};Upload.isEnhancedUpload=function(){var a=XMLHttpRequest?new XMLHttpRequest():null;return(a&&a.upload&&(FormData!=undefined));
};Upload.setProgress=function(a){if($("upload_progress")){try{if(a>=0){$("upload_progress").value=a;}else{$("upload_progress").removeProperty("value");
}}catch(b){}try{if(a>0){$("upload_progress").innerHTML=a+"%";}else{$("upload_progress").innerHTML="";
}}catch(b){}}};Upload.setMsg=function(a,b){if(b){a.setStyle("display","block");a.set("html",b);}else{a.setStyle("display","none");
}};Upload.setInfo=function(a){Upload.setMsg($("info_msg"),a);};Upload.setError=function(a){Upload.setMsg($("err_msg"),a);
};Upload.getIFrameBody=function(b){var c=b.contentDocument||b.contentWindow.document,a=c.getElementsByTagName("body");
return(a.length>0)?$(a[0]):null;};Upload.fileInList=function(d,b){if(d&&b){for(var c=0;c<b.length;c++){var a=b[c];
if(d.name&&a.name&&d.name===a.name){if(d.size&&a.size&&d.size!==a.size){continue;}if(d.lastModified&&a.lastModified&&d.lastModified!==a.lastModified){continue;
}return true;}}}return false;};function onFolderSelected(a){if(a==="/"||a==="\\"){a="";}form_clearErrors("uploadform");
