...
this.refreshZoomControls();}};ImgCanvasView.prototype.autoZoom=function(b){var a={type:"mousedown",pageX:Math.round(this.ctrOuterPos.x+this.ctrInnerPos.offsetLeft+(this.canvas.width/2)),pageY:Math.round(this.ctrOuterPos.y+this.ctrInnerPos.offsetTop+(this.canvas.height/2)),button:0,shiftKey:!b,api_event:true};
if(this.uiAttrs.fullScreen&&this.options.fullScreenFixed){a.pageX+=window.pageXOffset;a.pageY+=window.pageYOffset;
}this.onMouseDown(a);this.onMouseUp(a);};ImgCanvasView.prototype.stripTags=function(c,a){a=a||"";var b=new RegExp("</?"+a+"([^>]+)?>","gi");
...
};ImgCanvasView.prototype.getClickPosition=function(c,b){var g=QU.evPosition(c);var f=g.page.x-this.ctrOuterPos.x;
var e=g.page.y-this.ctrOuterPos.y;if(this.uiAttrs.fullScreen&&this.options.fullScreenFixed){f-=window.pageXOffset;
e-=window.pageYOffset;}f-=this.ctrInnerPos.offsetLeft;e-=this.ctrInnerPos.offsetTop;var d={x:Math.round8(f/this.canvas.width),y:Math.round8(e/this.canvas.height)};
if(b){var a=this.getViewportPosition();return{x:Math.round8(a.left+((a.right-a.left)*d.x)),y:Math.round8(a.top+((a.bottom-a.top)*d.y))};
}else{return d;}};ImgCanvasView.prototype.createControls=function(){var l;if(this.options.showcontrols=="auto"){l=document.createElement("div");
