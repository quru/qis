...
};function ImgGrid(b,i,d,j,c,a,f,h){this.initialised=false;this.destroyed=false;this.onInitialisedFn=h;
this.animating=false;this.g2d={ctx:c,origin:{x:0,y:0}};var e=Math.linearTween;switch(a.toLowerCase()){case"in-out-back":e=Math.easeInOutBack;
break;case"in-out-quadratic":e=Math.easeInOutQuad;break;case"in-out-sine":e=Math.easeInOutSine;break;
case"out-back":e=Math.easeOutBack;break;case"out-quadratic":e=Math.easeOutQuad;break;case"out-sine":e=Math.easeOutSine;
...
};}this.zoom={level:1,nextLevel:1,maxLevel:10,drawZoom:{x:1,y:1},animateFn:e};this.gridOpts={maxTiles:f,maxWidth:0,maxHeight:0,minWidth:0,minHeight:0,aspect:1,showGrid:false};
this.viewport={origWidth:b,origHeight:i,origAspect:b/i,width:b,height:i,aspect:b/i};this.grids=new Array();
this.grid=null;this.requests={queue:new Array(),active:0,limit:2,enforceLimit:true,requested:0,showProgress:false};
var g=d.indexOf("?");this.urlParams=QU.QueryStringToObject(d.substring(g+1),false);if(j){delete this.urlParams.halign;
delete this.urlParams.valign;}d=d.substring(0,g);g=d.lastIndexOf("/");this.urlBase=d.substring(0,g+1);
this.urlCommand=d.substring(g+1);this.loadingText="Loading image...";this.drawText(this.loadingText);
...
this.ctrEl.grab(this.canvas);this.layout();this.canvasContext=this.canvas.getContext("2d");if(!this.options.quality){this.canvas.setStyle("-ms-interpolation-mode","nearest-neighbor");
...
};ImgCanvasView.prototype.fullscreenKeydown=function(b){var a=(b.which||b.keyCode);if(b.code==27){setTimeout(this.toggleFullscreen.bind(this),1);
...
}.bind(a),1);}}function _get_image_src(a){if(a.src){return a.src;}var b=a.style.backgroundImage;if(b&&(b.length>5)&&(b.indexOf("url(")===0)){b=b.substring(4);
if((b.charAt(0)=="'")||(b.charAt(0)=='"')){return b.substring(1,b.length-2);}else{return b.substring(0,b.length-1);
}}return null;}function _img_fs_zoom_click(e,b,d){var c=_get_image_src(e);if(!c){return;}var a=QU.id("_cv_fs_zoom_click_el");
if(!a){a=document.createElement("div");a.id="_cv_fs_zoom_click_el";a.style.position="absolute";a.style.display="block";
a.style.width="500px";a.style.height="500px";a.style.left="-1000px";document.body.insertBefore(a,document.body.firstChild);
...
}var _hcvs=null;function haveCanvasSupport(){if(_hcvs==null){var a=document.createElement("canvas");_hcvs=(a&&a.getContext);
}return _hcvs;}function canvas_view_init(a,d,b,c){a=QU.id(a);if(a){if(haveCanvasSupport()&&QU.supported){if(a._cv_viewer!=undefined){a._cv_viewer.destroy();
...
if(a){if(d.title==undefined){d.title=a;}}if(d.stripaligns===undefined){d.stripaligns=true;}e._cv_click=function(){_img_fs_zoom_click(e,d,c);
};e.removeEventListener("click",e._cv_click,false);e.addEventListener("click",e._cv_click,false);}return false;
