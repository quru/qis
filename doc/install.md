...
but be aware that one of the mandatory dependencies - `qismagick` - is not currently
...
`Quru Image Server-2.xx.tar.gz` and `QIS-libs.tar.gz`.
...
Create the QIS base directory and install the files:
	...
	$ cd /opt/qis
	$ sudo -u qis tar -xvf ~/QIS-libs.tar.gz
